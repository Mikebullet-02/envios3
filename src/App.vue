<template>
  <Navbar></Navbar>
  <Header></Header>
  <div id="envios" class="flex flex-col w-full lg:px-52 md:px-0 sm:lg:px-0 px-0 my-auto">
    <Envios></Envios>
  </div>
  <div id="cobertura" class="flex flex-col w-full sm:w-full px-3 lg:px-52 md:px-0 sm:lg:px-0 my-auto">
    <h1 class="text-4xl font-black capitalize ml-5 sm:ml-24 py-14 sm:py-15">
      COBERTURA DE ENVÍOS
    </h1>
    <div class="mx-auto">
      <div class="flex flex-wrap justify-center pt-0 pb-5 sm:pb-8 sm:space-x-40">
        <div class="w-full md:w-1/2 lg:w-1/3 px-4 mb-8 md:mb-0">
          <select v-model="origen" @change="(e) => cargar(e)"
            ...
            <option v-for="destinos in databaseStore.$state.documents" :key="destinos.id" :value="destinos">
              {{ destinos.origen }}
            </option>
          </select>
          <div class="mt-4 flex items-center">
            <svg fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
              viewBox="0 0 24 24" class="text-blue-900 mr-4 w-16 h-16 sm:w-30 sm:h-30">
            ...
            <div>
              ...
              <p v-if="origen != null" class="text-xl sm:text-3xl text-[#003386]">
                {{ origen.ubicacion }}
              </p>
            </div>
          </div>
        ...
        <div class="w-full md:w-1/2 lg:w-1/3 px-4 mb-0 md:mb-0">
          <select v-model="destino"
            ...
            <option v-for="destinos in databaseStore.$state.destinos" :key="destinos.id" :value="destinos">
              {{ destinos.nombre }}
            </option>
          </select>
          <div class="mt-4 flex items-center">
            ...
            <div>
              ...
              <p v-if="destino != null" class="text-xl sm:text-3xl text-[#003386]">
                {{ destino.direccion }}
              </p>
            </div>
          </div>
    ...
    <div
      ...
      <div v-if="destino != null" class="bg-gray-800 p-4 rounded-lg text-center w-full sm:w-1/2">
        <p class="font-extrabold text-2xl sm:text-4xl text-center sm:pt-2 text-white">
          Sobres
        </p>
        <p class="font-extrabold text-xl sm:text-3xl text-center sm:pt-5 text-gray-300">
          Hasta 25 hojas
        </p>
        <p class="mt-0 sm:mt-4 text-3xl sm:text-4xl text-center sm:pt-0 sm:pb-0 text-gray-50">
      ...
      <div v-if="destino != null" class="bg-gray-800 p-4 rounded-lg text-center col-span-1 sm:col-span-2 w-full sm:w-1/2">
        <p class="font-extrabold text-2xl sm:text-4xl text-center sm:pt-2 text-white">
          Maletas
        </p>
        <p class="font-extrabold text-xl sm:text-3xl text-center sm:pt-5 text-gray-300">
  ...
  <div id="calcula" class="flex flex-col w-full lg:px-52 md:px-0 sm:lg:px-0 px-0 my-auto">
    <Calculadora></Calculadora>
  </div>
  <div id="tarifas" class="flex flex-col w-full lg:px-52 md:px-0 sm:lg:px-0 px-0 my-auto">
